...
const Profile = () => {
  ...
  useEffect(() => {
    setState({
      ...
      name: userInfo?.branchId?.name,
      mobile: userInfo?.branchId?.mobile,
      address: userInfo?.branchId?.address,
      description: userInfo?.branchId?.description,
      email: userInfo?.branchId?.email,
      division: userInfo?.branchId?.division,
      district: userInfo?.branchId?.district,
      sub_district: userInfo?.branchId?.sub_district,
      police_station: userInfo?.branchId?.police_station,
      post_code: userInfo?.branchId?.post_code,
    });
  }, [userInfo]);
  const add_image = (e) => {
            ...
            <div className="px-0 md:px-5 py-2">
              {view ? (
                <form onSubmit={add}>
                  ...
                  <div className="flex flex-col w-full gap-1 mb-3">
                    <label htmlFor="Shop">Branch Name</label>
                    <input
                      value={state.name}
                      onChange={inputHandle}
                      className="px-4 py-2 focus:border-indigo-500 outline-none bg-[#f1f3f7] border border-slate-700 rounded-md text-[#03050a]"
                ...
                <div className="flex justify-between text-sm flex-col gap-2 p-4 bg-slate-800 rounded-md relative">
                  ...
                  <div className="flex gap-2">
                    ...
                    <span>{userInfo.branchId?.name}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Address : </span>
                    <span>{userInfo.branchId?.address}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Mobile : </span>
                    <span>{userInfo.branchId?.mobile}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Email : </span>
                    <span>{userInfo.branchId?.email}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Description : </span>
                    <span>{userInfo.branchId?.description}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Division : </span>
                    <span>{userInfo.branchId?.division}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>District : </span>
                    <span>{userInfo.branchId?.district}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Sub District : </span>
                    <span>{userInfo.branchId?.sub_district}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Police Station : </span>
                    <span>{userInfo.branchId?.police_station}</span>
                  </div>
                  <div className="flex gap-2">
                    <span>Post Code : </span>
                    <span>{userInfo.branchId?.post_code}</span>
                  </div>
                </div>
              )}
            </div>
