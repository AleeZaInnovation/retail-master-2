...
const AccountType = () => {
  const { parties, accountType } = useSelector((state) => state?.transaction);
  const [impParties, setImpParties] = useState(parties);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < parties?.length; i++) {
      amount += parties[i]?.balance;
    }
    setBalance(amount);
    setTimeout(() => {
      setImpParties(parties);
    }, 500);
  }, [parties]);
  const handleZero = () => {
    const nParties = [];
            ...
            <div className="flex justify-between items-center pb-4 border-b border-gray-200 dark:border-gray-700 mb-3">
              ...
              <div className="flex flex-col">
                ...
                <div className="text-2xl  text-right dark:text-gray-300">
                  Balance up to {moment(Date.now()).format("ll")}
                </div>
              </div>
            </div>
            <div className="py-4">
